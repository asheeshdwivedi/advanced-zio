  ...
  def spec = suite("InterruptibilityRegions") {
    test("uninterruptible") {
      ...
      for {
        ...
        latch <- Promise.make[Nothing, Unit]
        fiber <- ZIO
                  .uninterruptible(latch.succeed(()) *> Live.live(ZIO.sleep(10.millis)) *> ref.update(_ + 1))
                  .forkDaemon
      ...
      test("interruptible") {
        ...
        for {
          ...
          latch <- Promise.make[Nothing, Unit]
          fiber <- ZIO
                    .uninterruptible(latch.succeed(()) *> ZIO.interruptible(ZIO.never))
    ...
    suite("BasicDerived") {
      ...
      test("ensuring") {
        def withFinalizer[R, E, A](zio: ZIO[R, E, A])(finalizer: UIO[Any]): ZIO[R, E, A] =
          withFinalizer2(zio)(_ => finalizer)

        def withFinalizer2[R, E, A](zio: ZIO[R, E, A])(finalizer: Exit[E, A] => UIO[Any]): ZIO[R, E, A] =
        ...
        test("acquireRelease") {
          def acquireReleaseWith[R, E, A, B](
            ...
            ZIO.uninterruptibleMask(restore => acquire.flatMap(a => restore(use(a)).ensuring(release(a))))
        ...
        def withFinalizer[R, E, A](zio: ZIO[R, E, A])(finalizer: UIO[Any]): ZIO[R, E, A] =
          ZIO.uninterruptibleMask { restore =>
            restore(zio).foldCauseZIO(
              error => finalizer *> ZIO.refailCause(error),
