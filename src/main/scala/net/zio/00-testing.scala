...
object SimplestSpec_v2 extends ZIOSpecDefault {

  def simpleTest = test("simple test") {
    val result = 2 + 2

    assertTrue(result == 4)
  }
  def simepleTest2 = test("simple test 2") {
    for {
      result <- ZIO.succeed(2 + 2)
    } yield assertTrue(result == 4)
  }
    ...
    test("incrementing a ref") {
      ...
      test("multiple assertions") {
        ...
        for {
          ...
          v    <- ref.updateAndGet(_ * 2).repeatN(rand)
        } yield assertTrue(v % 2 == 0 && v > 0)
      }
  }
}
  ...
  def spec =
    suite("TestServices") {
      ...
      test("TestClock") {
        ...
        test("TestSystem") {
          for {
            -    <- TestSystem.putEnv("name", "Sherlock Holmes")
        ...
        test("TestConsole") {
          for {
            -    <- TestConsole.feedLines("Sherlock Holmes")
...
object IntegrationSystem extends ZIOSpecDefault {
   ...
   */
  def spec = suite("IntegrationSystem")() @@ TestAspect.ifEnv("CI_MODE")(mode => mode == "STAGING")
}
...
object CustomLayers extends ZIOSpecDefault {
  ...
  final case class TestUserRepo(ref: Ref[Map[String, User]]) extends UserRepo {
     ...
     * in-memory test data stored in the Ref.
     */
    def updateUser(user: User): Task[Unit] = ref.update(_ + (user.id -> user))
  }
  ...
  lazy val testUserRepo: ULayer[UserRepo] = ZLayer {
    for {
      ref <- Ref.make[Map[String, User]](
              Map(
                "jhon@doe.com"        -> User("jhon@doe.com", "Johe Doe", 30),
                "jane@street.com"     -> User("jane@street.com", "Johe Street", 25),
                "sherlock@holmes.com" -> User("sherlock@holmes.com", "Sherlock Holmes", 43)
              )
            )
 ...
 *
 */
object DataGenerator extends ZIOSpecDefault {
  ...
  object DataProvider {
    ...
    def none[A: Tag]: ZLayer[Any, Nothing, DataProvider[Nothing]] =
      ZLayer.succeed(new DataProvider[Nothing] {
        override def get: UIO[Nothing] = ZIO.dieMessage("no provider")
      })
  }

  def dataGenerator[A: Tag](items: A*) = new TestAspectAtLeastR[DataProvider[A]] {
      ...
      items.map { item =>
        spec.provideSomeLayer[R] {
          ZLayer.succeed[DataProvider[A]] {
            new DataProvider[A] {
              def get: UIO[A] = ZIO.succeed(item)
            }
          }
        }
      }.reduceOption(_ + _)
  ...
  def spec =
    suite("DataGenerator") {
      test("List of int grater then 0") {
        for {
          data <- DataProvider.get[Int]
        } yield assertTrue(data > 0)
      } @@ dataGenerator(1, 2, 3)
    }.provideLayer(DataProvider.none[Int])
