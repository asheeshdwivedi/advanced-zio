    ...
    suite("QueueBasics") {
      ...
      test("offer/take") {
        for {
          ...
          take  <- queue.take.flatMap(a => ref.set(a)).fork
          _     <- offer.await *> take.await
          v     <- ref.get
        ...
        test("consumer") {
          for {
            ...
            producer <- ZIO.iterate(0)(_ => true)(int => queue.offer(int).as(int + 1)).fork
            consumer <- queue.take.flatMap(a => counter.update(_ + a)).repeatN(100).fork
            _        <- consumer.await
            value    <- counter.get
